        ...
        {totalBudgetRow ? (
          <>
            ...
            <div className="flex justify-between text-sm">
              <span className="opacity-80">사용 {formatCurrency(totalSpent)}</span>
          ...
          <p className="text-sm opacity-70">예산을 설정하면 지출 현황을 확인할 수 있어요</p>
        )}
      </div>

      {/* 카테고리별 지출 현황 */}
      <div className="flex-1 overflow-y-auto mt-3 px-4 pb-4 space-y-1">
        <p className="text-xs font-semibold text-gray-500 uppercase mb-2">카테고리별 지출</p>
        {Object.entries(spentByCat)
          ...
          .map(([catId, spent]) => {
            ...
            return (
              <div key={catId} className="bg-white rounded-2xl px-4 py-3 space-y-1.5">
                <div className="flex items-center gap-3">
                  <CategoryIcon icon={cat.icon} color={cat.color} size="sm" />
                  <span className="flex-1 text-sm font-medium">{cat.name}</span>
                  <span className="text-sm font-bold">{formatCurrency(spent)}</span>
                </div>
                {totalBudget > 0 && (
