...
export function ExpenseByMiddleCategory({
  transactions,
  categories,
  yearMonth,
}: Props) {
  ...
  const byMiddle = expenses.reduce<
    Record<
      string,
      { cat: Category; amount: number; txBySubCat: Record<string, number> }
    >
  >((acc, t) => {
    ...
    acc[middle.id].txBySubCat[subKey] =
      (acc[middle.id].txBySubCat[subKey] ?? 0) + t.amount;
    return acc;
  }, {});
          ...
          {rows.map((row) => {
            ...
            return (
              <React.Fragment key={row.cat.id}>
                <tr
                  className="border-b border-gray-50 cursor-pointer active:bg-gray-50 hover:bg-gray-50/60"
                  onClick={() =>
                    router.push(
                      `/home/transactions?types=expense&middleCategoryId=${row.cat.id}&month=${yearMonth}`,
                    )
                  }
                >
                  <td className="py-2">
                    <div className="flex items-center gap-1.5">
                      {expandable && (
                        <span
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setExpandedId(isExpanded ? null : row.cat.id);
                          }}
                        >
                ...
                {isExpanded &&
                  row.subRows.map((sub) => (
                    <tr
                      key={sub.id ?? sub.name}
                      className={`bg-gray-50/50 ${sub.id ? "cursor-pointer hover:bg-gray-100/60" : ""}`}
                      onClick={
                        sub.id
                          ? () =>
                              router.push(
                                `/home/transactions?types=expense&middleCategoryId=${sub.parentId}&subCategoryId=${sub.id}&month=${yearMonth}`,
                              )
                          : undefined
                      }
                    >
